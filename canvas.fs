...
let runApp
    ...
    SDL.SDL_CreateWindowAndRenderer(viewWidth, viewHeight, windowFlags, &window, &renderer)
    |> ignore
    ...
    let bufferPtr =
        IntPtr((Marshal.UnsafeAddrOfPinnedArrayElement(frameBuffer, 0)).ToPointer())
...
let runSimpleApp title width height (draw: int -> int -> canvas) : unit =
    runApp title width height (fun width height () -> draw width height) (fun _ _ -> None) ()
...
let rec turtleInterpreter
    (point: point, angle: int, color: color, isDrawing: bool)
    (cmds: turtleCmd list)
    (acc: line list)
    : line list =
    match cmds with
